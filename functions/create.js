    ...
    if (serviceId === "1") {
      ...
      if (shipRocketCreateOrderData.success) {
        ...
        if (shipRocketShipmentCreateData.id) {
          await connection.beginTransaction();
          await connection.execute('UPDATE SHIPMENTS set serviceId = ?, categoryId = ?, in_process = ?, shipping_vendor_reference_id = ? WHERE ord_id = ?', [serviceId, categoryId, true , shipRocketShipmentCreateData.id ,order])
